...
def toystock_list_test(
    ...
    # 테스트용: 이미지 URL을 리스트 형태로 변환
    for toy in _toystock_list:
        if isinstance(toy.image_url, dict):
            # 딕셔너리를 URL 리스트로 변환
            url_list = []
            if toy.image_url.get("main"):
                url_list.append(f"/api/toy_stock/images/{toy.image_url['main']}")
            for i in range(1, 10):  # sub1부터 sub9까지 체크
                sub_key = f"sub{i}"
                if toy.image_url.get(sub_key):
                    url_list.append(f"/api/toy_stock/images/{toy.image_url[sub_key]}")
            toy.image_url = url_list if url_list else None
        elif isinstance(toy.image_url, list):
            # 리스트를 URL 리스트로 변환
            if toy.image_url:
                url_list = [f"/api/toy_stock/images/{img}" for img in toy.image_url if img]
                toy.image_url = url_list if url_list else None
            else:
                toy.image_url = None
        elif isinstance(toy.image_url, str):
            # 문자열을 URL 리스트로 변환
            toy.image_url = [f"/api/toy_stock/images/{toy.image_url}"]
...
async def register_donation_bulk(
    ...
    if not toy_name or not toy_type or not soil or not damage or not is_donatable or not images:
        raise HTTPException(status_code=400, detail="필수 항목이 누락되었습니다.")

    if not (len(toy_name) == len(toy_type) == len(soil) == len(damage) == len(is_donatable)):
        raise HTTPException(status_code=400, detail="입력 데이터의 개수가 일치하지 않습니다.")
    ...
    for idx in range(len(toy_name)):
        ...
        try:
            ...
            errors.append(f"{toy_name[idx]}: is_donatable 값이 올바르지 않습니다. (impossible, recyclable, upcycle 중 선택)")
            continue
...
@router.get("/sale_list", response_model=toy_stock_schema.SaleListResponse)
async def get_sale_history(
    ...
    try:
        total, sale_list = toy_stock_crud.get_sale_history(
            db, 
